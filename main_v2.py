...
def loss(vmap_output_fn, graph, bools, activation, x, y, l1_lambda=0.01, l2_lambda=0.01):
    preds = vmap_output_fn(graph, bools, activation, x)
    
    # Cross-entropy loss
    ce_loss = -jnp.mean(y * jnp.log(preds + 1e-6))
    
    # L1 regularization
    l1_reg = l1_lambda * jnp.sum(jnp.abs(graph))
    
    # L2 regularization
    l2_reg = l2_lambda * jnp.sum(graph ** 2)
    
    # Sparsity penalty based on activation patterns
    sparsity_target = 0.05  # target sparsity level
    sparsity_penalty = jnp.sum(jnp.abs(jnp.mean(preds, axis=0) - sparsity_target))
    
    return ce_loss + l1_reg + l2_reg + (0.1 * sparsity_penalty)

def clip_gradients(grad_graph, max_norm=1.0):
    """Clip gradients to prevent exploding gradients"""
    grad_norm = jnp.linalg.norm(grad_graph)
    return jnp.where(
        grad_norm > max_norm,
        grad_graph * (max_norm / (grad_norm + 1e-6)),
        grad_graph
    )
...
def test_viz(graph, bools, acts, x, y, gen_num, is_best=False):
    ...
    def plot_data(x, preds, y, dataset_type, gen_num, accuracy):
        ...
        for i in range(len(x)):
            ...
            edge_color = correct_edge if preds[i] == y[i] else incorrect_edge
